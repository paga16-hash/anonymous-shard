...
export class DHTDiscoveryComponent implements DiscoveryComponent {
    ...
    addAddressMapping(address: string, port: number): void {
        ...
        const distance: number = this.xorDistance(this.id, address)
        const bucketIndex: number = Math.floor(Math.log2(distance)) || 0

        if (!this.buckets.has(bucketIndex)) {
            this.buckets.set(bucketIndex, new Map())
        }

        const bucket: Map<string, number> = this.buckets.get(bucketIndex)!
    ...
    async joinNetwork(): Promise<void> {
        ...
        const bootstrapAddress: string =
            bootstrapAddresses[Math.floor(Math.random() * bootstrapAddresses.length)]
    ...
    private async respondToDiscover(senderAddress: string): Promise<void> {
        ...
        const discoverResponse: DiscoveryResponseEvent = DiscoveryEventFactory.discoveryResponseEventFrom(
            this.id,
            peers
        )
    ...
    private startPing(): void {
        setInterval((): void => {
            ...
            peers.forEach((peer: string): void => {
                ...
                this.sender(peer, JSON.stringify(pingMessage)).catch((): void => {
