...
export class NodeImpl implements Node {
    ...
    constructor(address: string, port: number, bootstrapNodes: Map<string, number>) {
        ...
        this.dhtComponent = new DHTDiscoveryComponent(
            this.peerId(),
            this.bootstrapNodes,
            this.transportManager.sendToPeer.bind(this.transportManager)
        )
    ...
    async registerDiscoveryEventsHandler(
        handler: (discoveryEvent: DiscoveryEvent) => Promise<void>
    ): Promise<void> {
        try {
